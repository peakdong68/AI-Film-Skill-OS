---
name: director-core
description: AI 电影制作管线的总控制器——管理制作状态机、强制执行阶段锁定、维护检查点，并按正确顺序调度所有导演子技能。当用户想从创意或剧本制作完整的 AI 电影/视频时使用。触发场景：制作 AI 电影、从创意生成视频、导演 Seedance 项目、拍AI电影、制作AI视频、AI导演流程、film production pipeline，或任何多阶段视频创作需求。Master orchestrator for the AI film production pipeline — controls the production state machine, enforces phase locking, manages checkpoints, and routes to all director sub-skills in the correct sequence.
---

# Director Core — 制作状态机

## 概览

这是 AI Film OS 管线的总控制器。它确保每个 AI 电影/视频项目按顺序流经经过验证的、有阶段门控的制作序列——从初始创意到最终的 Seedance 就绪视频提示词。状态机防止了 AI 视频制作中最常见的失败模式：在故事、视觉语言和角色身份被锁定之前就跳到提示词生成。

此技能本身不生成创意内容。它在每个阶段将任务路由到相应的子技能，并在推进前验证输出。

## 加载资源

本技能附带以下参考知识文件，在相应场景下读取：
- 需要完整的状态流规则、阶段锁执行细节和多 Part 结构约束时，读取 `references/production-state-machine.md`

## 制作管线

```
STATE 0 → 输入采集
STATE 1 → 故事与情绪设计
STATE 2 → 视觉设计（摄影机 + 光影）
STATE 3 → 角色锁定
STATE 4 → 分镜规划
STATE 5 → 提示词编译
STATE 6 → 最终验证
STATE 7 → 导出就绪
```

每个状态产生经过验证的产出物后，下一个状态才解锁。严禁跳过状态。

## 阶段锁规则

这些是硬约束。违反它们会导致最常见的 AI 视频失败（角色漂移、视觉不一致、叙事不连贯）。

| 锁 | 规则 |
|------|------|
| **Story Lock（故事锁）** | 故事结构和情绪弧线必须在视觉设计前确认 | 
| **Visual Lock（视觉锁）** | 摄影机语言和光影系统必须在分镜前定义 |
| **Character Lock（角色锁）** | 角色身份表必须在提示词生成前确认 |
| **Storyboard Lock（分镜锁）** | 所有分镜帧必须在 Seedance 提示词前确认 |
| **Prompt Lock（提示词锁）** | 所有预检项必须在最终导出前通过 |

如果任何锁被打破，立即停止并返回到最早的未完成状态。

## 状态机

### STATE 0 — 输入采集

收集最小可行需求。只问开始所必需的内容：

- 项目创意或剧本（最少一句话）
- 预期时长（15s / 30s / 60s / 自定义）
- 视觉风格（cinematic / commercial / documentary / anime / sci-fi 等）
- 交付平台（Seedance / Runway / Sora / Kling）
- 画面比例（16:9 / 9:16 / 1:1）
- 任何已有的参考图或角色描述

如果用户提供的少于 4 项，询问缺失项。如果他们说 "直接生成"，用合理默认值填补并标注。

**State 0 输出**：一份确认的制作简报。进入 STATE 1。

### STATE 1 — 故事与情绪设计

路由到 `director-story` 和 `director-emotion`。

**必要产出物：**
- 叙事结构（3幕或5幕拆解）
- 场景清单（每个场景标明场景目的）
- 情绪弧线图（calm → tension → climax → resolution）
- 情绪强度时间线

**验证门：**
- [ ] 每个场景都有叙事目的
- [ ] 情绪弧线覆盖完整时长
- [ ] 因果链已定义（A 导致 B 导致 C）
- [ ] 用户已确认结构

**State 1 输出**：剧本蓝图 + 情绪时间线。进入 STATE 2。

### STATE 2 — 视觉设计

路由到 `director-camera` 和 `director-light`。

**必要产出物：**
- 摄影机语言蓝图（主要镜头类型、运动词汇、角度哲学）
- 光影设计系统（主光策略、色温弧线、对比度规则）
- 色彩脚本（每幕/场景的主导色、冷暖曲线）
- 构图规则（主要和辅助的取景方式）

**验证门：**
- [ ] 摄影机语言匹配情绪基调
- [ ] 光影随叙事演进
- [ ] 色彩脚本覆盖完整时长
- [ ] 用户已确认视觉语言

**State 2 输出**：视觉语言蓝图。进入 STATE 3。

### STATE 3 — 角色锁定

路由到 `director-character`。

**必要产出物：**
- 每个角色的角色身份表
- 视觉锁定参数（面部、发型、体型、服装、道具）
- 行为系统（动作签名、眼神逻辑、情绪→动作映射）
- 多角色关系图（如适用）

**验证门：**
- [ ] 所有角色都有身份锁定
- [ ] 视觉参数足够具体，可复现
- [ ] 行为系统覆盖情绪范围
- [ ] 用户已确认所有角色表

**State 3 输出**：角色表 + 身份锁定。进入 STATE 4。

### STATE 4 — 分镜规划

路由到 `storyboard-sketch`（用于 Seedance I2V）或 `storyboard-prompt` / `storyboard-master`（用于图像生成器分镜板）。

**必要产出物：**
- 分镜帧计划（每板 3-8 帧）
- 逐镜头拆解，含摄影机、动作、情绪、目的
- 所有帧的连续性锚点

**验证门：**
- [ ] 板数匹配计划时长
- [ ] 每帧都有叙事目的
- [ ] 角色身份在所有帧中保持一致
- [ ] 用户已确认所有分镜帧

**State 4 输出**：分镜板 + 镜头计划。进入 STATE 5。

### STATE 5 — 提示词编译

路由到 `director-seedance`。

**必要产出物：**
- 每个镜头的 Seedance 就绪视频提示词
- 镜头间的连续性上下文锁定
- 每个镜头的负面约束块
- 多 Part 连续性绑定（用于 >15s 的视频）

**预检清单（全部必须为 YES）：**
- [ ] 所有分镜已完成？
- [ ] 用户已确认所有分镜？
- [ ] 角色表已完成？
- [ ] 用户已确认角色？
- [ ] 视觉语言已定义？
- [ ] 时长和画幅比例已锁定？

如果任何答案为 NO，停止并返回到缺失的阶段。

**State 5 输出**：Seedance 视频提示词包。进入 STATE 6。

### STATE 6 — 最终验证

跨所有产出物进行质量检查：

- **叙事检查**：完整序列是否讲述了一个连贯的故事？
- **视觉检查**：视觉语言在所有镜头中是否一致？
- **角色检查**：每条提示词中角色身份是否得到保留？
- **连续性检查**：空间地理、光影和时间流是否感觉连续？
- **可执行检查**：每条提示词是否能直接在目标工具中使用？

**State 6 输出**：已验证的提示词包。进入 STATE 7。

### STATE 7 — 导出就绪

打包最终交付物：
- 按执行顺序排列的完整提示词列表
- 多 Part 生成的上下文连续性说明
- 参考图角色映射（如提供了参考图）
- 交付格式说明（时长、画幅比例、平台）

## 依赖关系图

```
director-story ────→ director-emotion
       │                    │
       └────────┬───────────┘
                ↓
         director-camera ──→ director-light
                │
                ↓
         director-character
                │
                ↓
         storyboard-sketch / storyboard-prompt / storyboard-master
                │
                ↓
         director-seedance
                │
                ↓
         [最终验证 → 导出]
```

## 路由指南

| 用户意图 | 首先加载 |
|---|---|
| "我有一个创意，把它拍成电影" | 保持在 director-core，从 STATE 0 开始 |
| "我已有剧本，需要视觉设计" | 从 STATE 2 进入 |
| "我已有分镜帧，需要 Seedance 提示词" | 从 STATE 5 进入 |
| "我只要角色表" | 直接路由到 director-character |
| "修复我的 AI 视频，角色一直变脸" | 从 STATE 3（重新锁定角色）进入，然后 STATE 5 |

## 状态追踪器

在每次制作会话结束时输出：

```markdown
**制作状态**
- 当前状态：[STATE N — 名称]
- 已完成状态：[列表]
- 待完成状态：[列表]
- 生效的锁：[哪些锁正在生效]
- 下一步行动：[用户需要做什么或确认什么]
```
