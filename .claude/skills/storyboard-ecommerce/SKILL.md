...
加载后探测可用上下文，确定素材来源和格式选择依据：

**1. 检查管线状态：** 读取 `STATE.md`，若存在则获取产品/创作者素材槽位。
...
**4. 无上下文时：** 按用户直接描述的产品信息选择格式。

---

## 子模式门

> **此技能不自动选择。** 仅当被 `director-core` STATE 5 明确路由、或用户明确请求电商分镜时才执行。不要在任何默认流程中自动触发。
...
- 所有参考图中的主体（创作者/模特/产品）必须保持严格的身份一致性——不得发明变体。参考素材使用 `@[图片N]` 格式指代
- 镜头卡片必须包含所有必填字段：编号、景别、时间码、预览、动作、摄影机
- 视觉风格关键词必须清晰描述板面格式——而非视频内容
- 压缩提示词中必须包含反模式清单，防止 AI 图像生成器漂移到海报/概念图风格
- 若用户提供产品/创作者参考图，精确保持其视觉身份
- **有参考图时，禁止在 Replaceable Zone 和 Compressed Prompt 中逐项复述主体外貌（面部、发型、体型、服装细节）。参考图已锁定视觉身份——仅需槽位引用 + 1-2 个标识。复述已在图中的细节会导致生图模型重新解读而非直接引用。**
...
```
保存到 outputs/F-[N]-[topic]-ecommerce-board.md？
