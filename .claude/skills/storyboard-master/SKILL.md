...
**角色锚点声明：** 当上游已定义素材槽位时，在分镜网格上方声明角色素材映射：

```
角色锚点：所有镜头中[角色A]形象引用@[图片1]（[角色A]角色设定图），[角色B]形象引用@[图片2]（[角色B]角色设定图）。
```
...
镜头卡片信息格式: `编号 | 景别 | 时间码`
角色引用格式: `Ref: @[图片N] @[图片M]`（标注此镜头依赖的角色/素材槽位）

**规则：** 当上游 `director-core` 已定义 Material Slot Registry（素材槽位注册表）时，每个镜头卡片必须标注其依赖的角色槽位。这确保分镜蓝图图像中每个镜头画面能引用正确的角色形象。
