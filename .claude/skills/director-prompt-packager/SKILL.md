...
> **STATE 5 只产出 AI 图像生成器提示词。绝不产出视频平台提示词。**

此硬约束防止最常见的管线错误——将图像提示词误输入 Seedance 2.0 视频平台。

**严禁在 STATE 5 的任何输出中提及以下视频平台名称：**
...
- "将此提示词输入 即梦/MJ/Flux/可灵图像模式 生成分镜帧图像"
- "生成的分镜图将作为 STATE 6（seedance-video-prompt）的 @[ref] 输入"

违反此边界将导致用户将图像提示词误输入视频平台，造成生成失败或产出异常。STATE 5 → STATE 6 的职责分离是管线设计的基础，绝不可跨越。
...
### 规则 7：输出平台边界
- 每条提示词末尾的生成指令**必须明确指向图像生成器**（即梦/MJ/Flux/可灵图像模式）
- 绝不可写成"在 Seedance 中生成"或"Seedance 2.0 提示"
- 若使用中文即梦/可灵，以 `--ar 9:16 --style raw` 等图像平台参数收尾
...
- [ ] 时长已指定
- [ ] 提示词可直接用于 AI 图像生成器
- [ ] **输出边界合规：未提及 Seedance / Runway / Sora / Kling 等视频平台**
- [ ] **生成指令指向图像生成平台（即梦/MJ/Flux/可灵），非视频平台**
...
| 平台 | 提示词风格 | 建议字数 |
|------|------|-----|
| **Midjourney** | 简洁英文，视觉关键词优先 | 英文 ≤ 200 词 |
| **Flux** | 自然语言描述，细节丰富 | 英文 ≤ 300 词 |
| **即梦/可灵（图像模式）** | 优先中文，强画面感 | 中文 ≤ 300 字 |
| **GPT Image** | 结构化描述，规避敏感词 | 英文 ≤ 250 词 |

## Anti-Slop 去水词

在图像生成提示词中替换空洞评价为可观察描述：

| 空洞词 | 替换为 |
|------|-----|
| 电影感 / cinematic | 具体景别 + 摄影机运动 + 光源方向 |
| 高级 / premium | 材质表现 + 光影层次 + 构图方式 |
| 质感 / texture | 面料纤维可见 + 光线在材质表面流动 |
| 氛围感 / atmospheric | 光源色温 + 景深 + 环境动态元素 |

规则：如无法被相机、灯光、或物理动作检测到，则重写。

## 下游衔接

此技能产出文本级提示词包后，工作流继续：

1. 用户将提示词输入 **AI 图像生成器**（即梦/MJ/Flux/可灵） → 生成分镜帧图像
