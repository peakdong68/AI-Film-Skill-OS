...
| `State-4-prompt-package.md` | 提取完整分镜方案作为 I2V 帧规划的数据源        |
| `State-3-characters.md`     | 提取角色锁定参数确保跨帧身份一致性             |
| `State-2-visual.md`         | 提取灯光色彩方案指导每帧的 Lighting & Mood     |
| `State-1-story-emotion.md`  | 提取情绪弧线决定帧节奏分配                     |

**3. 自动补全：** 若探测到角色设定图文件，在 `Continuity anchors` 中引用 `@[图片N]` 槽位；在每帧 `I2V motion note` 中嵌入角色引用。
...
> **此技能不自动选择。** 仅当被 `director-core` STATE 5 明确路由、或用户明确要求 `storyboard-sketch` 时才执行。不要在任何默认流程中自动触发此技能。

生成输出前，决定使用哪种模式：

| 用户说...                                                                                | 使用此模式                      |
| ---------------------------------------------------------------------------------------- | ------------------------------- |
| "分镜草图", "keyframe prompts", "I2V frame planning", "逐帧动作描述", "per-frame motion" | **Compact Frame Prompts**       |
...
- **计划部分**：使用用户的项目语言。中文项目 = 中文计划，英文项目 = 英文计划。不得混用。
- **压缩提示词**：使用 AI 图像生成器的首选语言（英文通常效果更稳定），但其中的镜头动作描述和场景描述必须与计划部分使用相同语言。
