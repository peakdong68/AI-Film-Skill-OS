...
description: '将 director-character 产出的角色身份定义编译为多视角角色设计板（Character Sheet）提示词，适配 AI 图像生成平台（MJ/Flux/即梦/GPT Image）。产出用于 AI 视频一致性控制的角色设定图。当用户已有角色参考图像或 director-character 角色定义，需要生成可在生图平台使用的 Character Sheet 提示词时使用。亦在 director-core 于 STATE 3 完成后路由至此。'
---
...
> 角色设定参考图 = **多视角角色设计板（Character Sheet）**——用于 AI 视频跨镜头一致性控制。
>
> 本技能产出的是 **Character Sheet 生图提示词**，风格由项目上下文灵活推导（电影级写实、艺术感、概念设计等均可）。目的是锁定角色身份，贯穿所有生成视频帧。
...
Character Sheet 提示词是平台无关的。按目标平台调整语言和参数：

| 平台              | 推荐语言      | 说明                           | 参数                            |
| ----------------- | ------------- | ------------------------------ | ------------------------------- |
| **Midjourney**    | 英文          | 英文提示词效果最佳             | `--ar 16:9 --style raw --v 6.1` |
| **Flux**          | 英文          | 英文提示词，自然语流           | 无                              |
| **GPT Image**     | 中文/英文均可 | 两种语言均支持，按用户习惯选择 | 无                              |
| **即梦 (Jimeng)** | 中文/英文均可 | 中文提示词理解良好，英文亦可   | 无                              |
| **可灵 (Kling)**  | 中文/英文均可 | 中文提示词理解良好，英文亦可   | 无                              |

> **建议：** Midjourney/Flux 优先使用英文；即梦/可灵/GPT Image 按用户平台语言习惯选择，无需强制翻译。
...
- [ ] 背景为中性，不干扰角色识别
- [ ] 产出物为多视角角色设计板，用语匹配项目风格（电影级/艺术感/概念设计等均可）
...
| STATE 3 路径 | 输入                                        | 模式               |
| ------------ | ------------------------------------------- | ------------------ |
| 路径 A       | 用户已有参考图                              | 模式 A（有参考图） |
| 路径 B       | `director-character` 产出的角色身份文本定义 | 模式 B（无参考图） |

**流程：**

1. 加载角色身份（路径 A 用参考图、路径 B 用 `director-character` 文本定义）
2. 编译为 [0]-[12] 完整 Character Sheet Profile + Image Prompt
3. 用户确认后，将 [11] 粘贴到选择的 AI 生图平台生成 Character Sheet 图像
4. 生成的 Character Sheet 作为 `@[图片N]` 参考输入用于 STATE 6 `seedance-video-prompt`
...
```
保存到 outputs/F-[N]-[topic]-character-sheet.md？
