...
**中文：**
```
使用@[图片1]为参考；精确保留[身份/产品/场景]。仅[运动]发生变化。
镜头：[单一运镜]。光影：[光源或过渡]。音效：[提示音]。约束：[不变项]。
```

**英文：**
```
use @[Image1] as reference, strictly preserve [identity/product/scene].
...
**示例——产品：**
```
使用@[图片1]为产品参考；精确保留瓶身标签、Logo、瓶型、颜色不变。
...
使用@[视频1]仅控制侧向平移的摄影机节奏；不复制表演者、室内布局或品牌标识。
使用@[音频1]仅控制节奏和能量。
...
| ID 漂移/换脸 | 使用面部特写（大头照）+ 全身照分离，提升面部特征提取权重 |
...
**紧凑中文 I2V 模板：**
```
使用@[图片1]为参考，严格保持[主体]不变；仅加入[动作/光线/镜头]。音效：[提示]。约束：[不变项]。
```
